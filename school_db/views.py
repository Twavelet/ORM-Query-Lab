...
def problem_one(request):
  # students = Student.objects.all()
  b_students = Student.objects.filter(gpa__gt=3.0).order_by('-gpa')
  for student in b_students:
    print(f"{student.first_name} {student.last_name}'s gpa is {student.gpa}.")
...
def problem_two(request):
  ...
  hired_instructors = instructors.filter(hire_date__year__lt = 2010).order_by('hire_date')
  for instructor in hired_instructors:
    print(f"{instructor.first_name} {instructor.last_name} was hired in {instructor.hire_date}.")
...
def problem_five(request):
    new_student = Student.objects.create(first_name = "Trevor", last_name = "Wavelet", year = 11, gpa = 4.0)
...
def problem_six(request):
  Student.objects.filter(pk = 13).update(gpa = 4.1)
  new_student = Student.objects.get(pk = 13)
  # new_student.get(pk = 13)
  print(f"ID: {new_student.id}\nFull Name: {new_student.first_name} {new_student.last_name}\nGPA: {new_student.gpa}")
...
def problem_seven(request):
    Student.objects.get(pk=13).delete()
    # Make sure to set this equal to the primary key of the row you just created!
    student_id = 13
...
def bonus_problem(request):
    Course.objects.annotate(course_count=Count('course.id'))
      return complete(request)
